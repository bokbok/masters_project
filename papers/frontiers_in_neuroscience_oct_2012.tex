...
\author{Matthew Walsh\\
   ...
   \small matthew.walsh@gmail.com\\
  \and
    David Liley\\
   \small  Brain and Psychological Sciences Research Centre (BPsyC),\\
   \small Swinburne University of Technology, Hawthorn, Victoria 3122,\\
   \small Australia; Cortical Dynamics Ltd, Hawthorn. Victoria 3122, Australia\\
   \small dliley@swin.edu.au\\
}
...
The ratio of of the characteristic timescales of the fast and slow oscillations($\mu$) is given as $\mu \ll 0$. 

Bursting can be described by a fast slow system with the general form:
...
With $x \in \mathbb{R}^m, u \in \mathbb{R}^n$ being vectors of the fast and slow system variables respectively. 

Equation \ref{slow_gen_form} describes a slow system that is driven by the fast system. Equation \ref{fast_gen_form} describes a fast system that is modulated by the slow system.


\subsection{Fast dynamics within the current model of Liley Et Al}
...
The inhibitory population activity phase leads excitatory population activity and thus describes a phase locked system of inhibitory and excitatory neurons. 

The phase locking of these systems leads us to assume that equations \ref{h_k} and \ref{i_lk} represent a single timescale with the model of Liley et al and as such were considered to be a 'fast' system within the exploration.
...
\end{equation}
with $\tau^{slow}$ being the time constant for the slow system and $g$ the feed forward connectivity constant.
...
Equation \ref{eq:a_ek} includes the constants $p_{ek}$ representing the rate of extra cortical input.

$p_{ee}, p_{ei}$ can be reformulated to include a mean field driven deviation from its average (constant) value. The extracortical inputs ($p_{ek}$) then become

\begin{equation}
p_{ek} = \hat{p_{ek}} + \rho_k \zeta
\end{equation}

Where $k = e,i$, $\rho$ is the thalamic feedback connectivity constant and $\hat{p_{ek}}$

\subsection{Modelling intracellular and/or synaptic resource utilisation}
Depletion of synaptic resources is assumed to result in a depolarising of neurons (ELABORATE, REFERENCE). This depolarisation can be expressed as a deviation of the resting potential $h_{k}^{rest}$. The resting potential was reformulated to become

\begin{equation}
h_{j}^{rest} = \hat{h_{j}^{rest}} + \chi \zeta
\end{equation}

Where $\hat{h_{j}^{rest}}$ is a constant representing the average resting potential and $\chi$ is the resource utilisation coupling constant. 

\subsection{Slow system emerges from parametric variation of long range coupling}
Literature suggests that anaesthesia may effect conduction axonal velocities (REF?), demylenated fibres stuff here. We investigated the effects of that modifying the conduction velocity ($v$ and $\Gamma_{ek}$) had on the long range coupling ($\phi_{ek}$) were investigated. A reduction of $v$ and $\Gamma_{ek}$ 
showed that the long range coupling can operate on a timescale slower and independent to the timescale of the coupled excitatory/inhibitory system.

\subsection{Computational methods}
We performed all numerical integrations of the aforementioned models using the Python programming language, utilising the excellent PyDSTool library under 64 bit Linux. The Radau integrator was used for all integrations. Typical numerical settings included step size, ......

\section{Results}
\subsection{Thalamic coupled bursting}
\subsection{Resource utilisation driven bursting}

\subsection{Bursting observed in cannonical model}

\section{Discussion}
\subsection{Multiple origins for bursting possible}
\subsection{Long range coupling}
Mention it may account for synchronous bursting observed in EEG, need to scale up to PDEs and investigate
\subsection{Onset may emerge from parametric changes in the 'fast' system alone}

...
cont = equib.searchForBifurcations('phi_c', 'h_e', steps = 2000, maxStepSize = 1).display(fig = "3")
...
if len(hopf) >= hopf_count and len(saddlenode) >= saddle_node_count:
    ...
    for point in hopf:
        print point
        file.write(">> Hopf at phi_c=" + str(cont.specialPoints(point)['phi_c']) + "\n")

    for point in saddlenode:
        file.write(">> saddlenode at phi_c=" + str(cont.specialPoints(point)['phi_c']) + "\n")
